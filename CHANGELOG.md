...
### Added
- `--markdown` flag on `comment create`. ClickUp's v2 comment API has two body shapes: `comment_text` (a plain string, stored verbatim — markdown syntax appears as literal `#`, `**`, etc. in the UI) and `comment` (a flat Quill-delta-style array of "ops" that ClickUp renders as rich content). The CLI previously only sent `comment_text`. With `--markdown`, the `--text` value is parsed as markdown (via `comrak`) and posted as a `comment` op stream, so it renders as formatted content. Each op is `{ "text": "…", "attributes"?: {…} }`: inline formatting rides on the text op (`bold`, `italic`, `code`), while block formatting rides on the newline that terminates each line — `{ "header": 1|2|3 }` for `#`/`##`/`###`, `{ "list": "bullet"|"ordered" }` (with `indent` for nesting) for lists, `{ "blockquote": true }` for `> `, and `{ "code-block": true }` for fenced/indented code. Without the flag, behavior is unchanged (plain `comment_text`).
