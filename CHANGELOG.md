...
### Added
- `--markdown` flag on `comment create`. ClickUp's v2 comment API has two body shapes: `comment_text` (a plain string, stored verbatim — markdown syntax appears as literal `#`, `**`, etc. in the UI) and `comment` (an array of doc blocks that ClickUp renders as rich content). The CLI previously only sent `comment_text`. With `--markdown`, the `--text` value is parsed as markdown and posted as `comment` doc blocks, so it renders as formatted content. Supported blocks: `h1`/`h2`/`h3` (`#`/`##`/`###`), `bullet_list` (`- `/`* `), `ordered_list` (`1. `), fenced `code` (with an optional language attr), `blockquote` (`> `), and `p` for everything else; inline formatting (`**bold**`, `*italic*`, `` `code` ``) is left intact for ClickUp to render. The parser is a small dependency-free line-based converter. Without the flag, behavior is unchanged (plain `comment_text`).
